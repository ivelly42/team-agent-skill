...
# 3-tier timeout 래퍼 (refs/timeout-wrapper.sh와 동일 구현, 다른 사용처와 일관).
...
# bug_006: 전부 실패 시 빈 문자열 + rc=1. 반드시 확인.
_GEMINI_VERIFIER_MODEL="$(_pick_gemini_model verifier)"
if [ -z "$_GEMINI_VERIFIER_MODEL" ]; then
  echo "[gemini-verification] verifier 모델 탐색 실패 (전 후보 404/429) — 검증 스킵" >&2
  _GEMINI_RC=127
else
