...
```bash
python3 <<PYEOF > /tmp/ta-${_RUN_ID}-config.env
import json, os, shlex, sys

SKILL_DIR = os.environ.get("_SKILL_DIR", "${_SKILL_DIR}".replace("\${_SKILL_DIR}", ""))
...
# 셸 변수 exports — 주요 knob만 평면화
def q(v): return shlex.quote(str(v))
t = cfg["timeouts"]; lim = cfg["limits"]; w = cfg["weights"]
oh = cfg["cost_overhead_k_tokens"]; b = cfg["batch"]
gm = cfg["gemini"]

print(f"export _CFG_AGENT_SOFT_SEC={q(t['agent_soft_sec'])}")
print(f"export _CFG_VERIFY_SEC={q(t['verify_sec'])}")
print(f"export _CFG_CODEMAP_SEC={q(t['codemap_sec'])}")
print(f"export _CFG_GRACE_SEC={q(t['grace_sec'])}")
print(f"export _CFG_TASK_PURPOSE_CHARS={q(lim['task_purpose_chars'])}")
print(f"export _CFG_PROJECT_CONTEXT_CHARS={q(lim['project_context_chars'])}")
print(f"export _CFG_ROLE_FILTERED_CHARS={q(lim['role_filtered_context_chars'])}")
print(f"export _CFG_CONSOLIDATOR_CHARS={q(lim['consolidator_input_chars'])}")
print(f"export _CFG_VERIFY_CAP={q(lim['verify_target_cap'])}")
print(f"export _CFG_WEIGHT_PRECISE={q(w['precise'])}")
print(f"export _CFG_WEIGHT_STRUCTURE={q(w['structure'])}")
print(f"export _CFG_WEIGHT_DOCS={q(w['docs'])}")
print(f"export _CFG_WEIGHT_EXPLORE={q(w['explore'])}")
print(f"export _CFG_OVERHEAD_CODEX={q(oh['codex'])}")
print(f"export _CFG_OVERHEAD_GEMINI={q(oh['gemini'])}")
print(f"export _CFG_OVERHEAD_OPUS={q(oh['opus_consolidator'])}")
print(f"export _CFG_BATCH_SMALL={q(b['size_small'])}")
print(f"export _CFG_BATCH_LARGE={q(b['size_large'])}")
print(f"export _CFG_BATCH_SLEEP_SEC={q(b['sleep_between_sec'])}")
# Gemini alias 후보 배열 — 공백 구분 문자열로 export (첫 매치가 최우선)
print(f"export _CFG_GEMINI_AGENT_CANDIDATES={q(' '.join(gm['candidates_agent']))}")
print(f"export _CFG_GEMINI_VERIFIER_CANDIDATES={q(' '.join(gm['candidates_verifier']))}")
PYEOF
source /tmp/ta-${_RUN_ID}-config.env
rm -f /tmp/ta-${_RUN_ID}-config.env
```
...
예시:
  ...
  /team-agent --ultra=selective 감사      (가중치 선별 3중, 44% 절감)
...
- `--ultra [full|selective]` → `ULTRA_MODE=true` + `ULTRA_STRATEGY=full|selective` (기본: `full`). `--codex`·`--gemini`·`--cross`와 상호 배타. codex·gemini CLI 양쪽 필요 (미설치 시 자동 다운그레이드). 파싱: `--ultra=selective` 또는 `--ultra selective` 양쪽 허용.
  ...
  - `selective`: 가중치별 선별 복제. ×1.5(정밀) → 3중, ×1.0(구조) → 2중(Claude+Codex), ≤×0.7(문서·탐색) → 1중(Claude). 5역할 기준 ~44% 토큰 절감.
  - Phase 2.5에서 역할별 Opus 통합자가 결과를 합성 (3/3·2/3·1/3·1/1 합의 계산, 모순 감지). selective에선 `agreement` enum에 `1/1`·`2/2`도 정상 값.
...
```
def ultra_replication(role_weight: float, strategy: str) -> list[str]:
    """역할 가중치와 strategy로 spawn할 백엔드 리스트 결정. 가용성 필터는 호출측."""
    if strategy == "full":
        return ["claude", "codex", "gemini"]  # 전원 3중 (기본)
    # strategy == "selective"
    if role_weight >= 1.5:
        return ["claude", "codex", "gemini"]  # 정밀 역할만 3중
    if role_weight >= 1.0:
        return ["claude", "codex"]  # 구조 역할은 2중
    return ["claude"]  # 문서/탐색(≤0.7)은 1중
```

각 역할(`team.roles`)마다 위 함수 결과로 **ULTRA_CODEX_AVAIL/ULTRA_GEMINI_AVAIL 필터링 후** 실제 spawn할 백엔드 집합을 결정하고, 결정 결과마다 프롬프트 변형을 생성한다:
...
**selective에서 Codex/Gemini 미설치 다운그레이드**:
- `ULTRA_CODEX_AVAIL=0` + 가중치 ≥1.0 역할 → Claude 단독 (2중 → 1중)
- `ULTRA_GEMINI_AVAIL=0` + 가중치 ≥1.5 역할 → Claude+Codex 2중 (3중 → 2중)
- 양쪽 모두 미설치 → Claude 단독 (모든 가중치)
...
- 최대 동시성은 strategy와 역할 수에 따라 달라진다:
  ...
  - `selective` 5역할 (×1.5 1명·×1.0 2명·×0.7 2명) = Claude 5 + Codex 3 + Gemini 1 = **9명** (~40% 감소)
...
**selective 1중 역할 패스스루 (통합자 생략)**:

`ULTRA_STRATEGY="selective"`이고 해당 역할의 `agent_groups[{역할}]`에 백엔드가 1개만 기록된 경우, 통합자를 **spawn하지 않고** 단일 결과를 직접 `per_role_integration[{역할}]`에 기록한다:
