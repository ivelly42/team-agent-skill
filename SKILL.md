...
- gemini-3.1-flash-lite 단가: 입력 $0.25/MTok, 출력 $1.50/MTok (2.5-flash 대비 2.5배 빠른 TTFT, 2026-03 출시)
...
- 비용 표시 시 `[Claude]` / `[Codex]` / `[Gemini]` 태그와 함께 에이전트별 예상 달러를 병기한다
...
(`--cross` 모드에서만 "합의" 컬럼 표시 — 3/3, 2/3, 다운그레이드, 쟁점 등. 다른 모드에서는 "-" 또는 생략.)

### 쟁점 항목 (2/3 불일치, `--cross` 모드만)
