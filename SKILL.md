...
- `---BEGIN_USER_INPUT---`, `---END_USER_INPUT---` 시퀀스
- `"`, `\`, 제어문자(0x00-0x1F)
- 100자 초과 시 잘라냄
...
- 발견한 비밀값(API 키, 토큰, 비밀번호, 연결 문자열)의 **원문을 인용하지 말라**. 위치와 유형만 보고하라. 예: "src/config.ts:12에 하드코딩된 API 키 존재" (키 값 자체는 포함 금지)
...
심각도는 반드시 다음 중 택 1 (한글 금지): Critical / High / Medium / Low / Info
...
**slug 생성**: TASK_PURPOSE에서 영문 키워드 추출 후, 고유성을 위해 타임스탬프 해시를 추가:
```bash
SLUG_BASE=$(echo "영문키워드" | tr '[:upper:]' '[:lower:]' | tr -dc 'a-z0-9-' | sed 's/--*/-/g; s/^-//; s/-$//' | cut -c1-16)
[ -z "$SLUG_BASE" ] && SLUG_BASE="task"
SLUG="${SLUG_BASE}-$(date +%H%M%S | cut -c1-4)"
echo "SLUG: $SLUG"
```
**한국어 목적 처리**: 영문 키워드가 없어 SLUG_BASE가 비어있으면, TASK_PURPOSE를 영문 약어로 번역하여 slug에 반영한다. 예: "보안 점검" → "security", "성능 최적화" → "perf-opt". 번역이 어려우면 "task"를 사용. HHMMSS 타임스탬프로 동일 날짜 재실행 시에도 slug가 고유.
...
Step 3에서 결정된 에이전트 수만큼 Agent 도구를 **한 번에 병렬 호출**한다. 각 Agent 호출에 다음 파라미터를 지정:
...
Phase 2에서 수집된 결과를 기반으로 교차 검증 에이전트를 1명 추가 생성:
- Agent 도구로 `cross-validator` 에이전트를 생성
- prompt에 Phase 2의 모든 에이전트 결과를 포함
- 에이전트 간 모순, 누락, 심각도 불일치를 검증
- 결과를 Phase 4 보고서에 반영
...
```
전원 실패 시에도 4-B(파일 저장)와 4-C(히스토리 기록)는 실행한다. 보고서에 "전원 실패" 상태를 기록하고, .history.json에 `success_count: 0`, `fail_count: {N}`, `findings_summary: []`로 저장.
...
`time`: 실행 시작 시각 (HHMMSS). 같은 날 동일 slug 실행 구분용.
`duration_min`: Phase 0 시작 ~ Phase 4 완료까지 소요 시간(분, 소수점 1자리).
`findings_summary`: 품질 필터 적용 후 발견 사항 제목 리스트 (아이디어 제외).

**예외 처리**: `.history.json` 읽기 시 파일이 손상되었거나 JSON 파싱에 실패하면, 빈 배열 `[]`로 초기화하고 계속 진행. 에러를 사용자에게 안내하되 스킬을 중단하지 않는다.

하위 호환: 이전 기록에 `time`, `duration_min`, `findings_summary`가 없으면 각각 `null`, `null`, `[]` 처리.
