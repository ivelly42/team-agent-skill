---
name: team-agent
argument-hint: "[--auto] [--deep] [--dry-run] [--codex [all|hybrid]] [--scope <path>] [--resume <RUN_ID>] [--notify telegram] <작업 목적>"
description: |
...
|---|------|---------|----------|
| 1 | 보안 감사 | 보안 | `## 보안` |
| 2 | Python 전문가 | 언어 | `## 언어 전문가` |
| 3 | TypeScript 전문가 | 언어 | `## 언어 전문가` |
| 4 | 백엔드 아키텍트 | 아키텍처 | `## 아키텍처` |
| 5 | 프론트엔드 개발자 | 개발 | `## 프론트엔드` |
| 6 | DB 아키텍트 | 데이터 | `## DB` |
| 7 | 성능 엔지니어 | 성능 | `## 성능` |
| 8 | AI/ML 엔지니어 | AI | `## AI/ML` |
| 9 | 디버거 | 디버깅 | `## 디버깅` |
| 10 | 클라우드 아키텍트 | 인프라 | `## 인프라/배포` |
| 11 | 배포 엔지니어 | 인프라 | `## 인프라/배포` |
| 12 | 문서 아키텍트 | 문서 | `## 문서` |
| 13 | TDD 오케스트레이터 | 테스트 | `## 테스트` |
| 14 | UI/UX 디자이너 | 디자인 | `## UI/UX` |
| 15 | 장애 대응 전문가 | 운영 | `## 장애 대응` |
| 16 | 코드 리뷰어 | 품질 | `## 코드 품질` |
| 17 | 코드 탐색가 | 분석 | (프로젝트 구조 파악 전담) |
| 18 | 통합 정합성 검증 (Integration QA) | QA | `{SKILL_DIR}/refs/integration-qa.md` 전문 |

> **에이전트 수**: 3~8명 (기본 5~6명). 7명 이상 시 비용/시간 경고 표시.

> **통합 정합성 검증 에이전트 (#18)**: 프론트엔드+백엔드 API가 동시 존재하는 프로젝트에서 **자동으로 팀에 포함**한다. Step 2에서 `package.json`(또는 tsconfig.json) + `src/app/api/` (또는 pages/api/, routes/) 패턴이 동시 감지되면 활성화. 이 에이전트는 개별 영역이 아니라 **경계면(API 응답 shape ↔ 프론트 훅 타입, 라우팅 경로 ↔ 링크 href, 상태 전이 맵 ↔ 실제 코드)**만 전담 교차 비교한다. 상세 체크리스트는 `refs/integration-qa.md`를 Read하여 프롬프트에 포함.
...
**LLM 구성 규칙**: `RUN_ID_VALUE`, `DATE_VALUE`, `HHMMSS_VALUE`, `AGENT_MODE_VALUE`, `MANIFEST_PATH` 등 Preamble/Phase 0에서 얻은 시스템 값과 `AUTO_MODE_BOOL`, `DEEP_MODE_BOOL`, `NOTIFY_BOOL`, `CODEX_MODE_OR_NONE` 등 LLM 내부 플래그는 Python 리터럴로 직접 치환한다. `MANIFEST_PATH`는 Phase 0에서 정의한 `$_MANIFEST` 값(예: `docs/team-agent/.runs/2026-04-06-001534.json`)으로 치환한다. `TASK_PURPOSE`, `PROJECT_CONTEXT`, `PROJECT_DIR`, `PROJECT_NAME`, `SCOPE_PATH`처럼 사용자 유래 또는 외부 유래 값은 **Write 도구로 파일에 저장**하고 Python에서 `json.load()`로 읽는다. 이 패턴은 사용자 입력이 셸 명령에 절대 삽입되지 않으므로 인젝션을 원천 차단한다.
...
**역할별 토큰 가중치**:
- 정밀 분석 (보안 감사, 디버거, 성능 엔지니어): ×1.5
- 구조 분석 (백엔드 아키텍트, 클라우드 아키텍트, 코드 리뷰어): ×1.0
- 문서/디자인 (문서 아키텍트, UI/UX 디자이너): ×0.7
- 탐색/QA (코드 탐색가, 통합 정합성 검증): ×0.5
