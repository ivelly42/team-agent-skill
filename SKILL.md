...
**fail-closed 원칙**: Python 로더·source 어느 한쪽이라도 실패하면 즉시 `exit 1`. 사용자 오버라이드가 조용히 무시되어 하드코딩 default로 회귀하는 drift를 차단한다.

**cross-invocation 지속 설계 (필수 — bug_007 대응)**: Claude Code Bash 도구는 호출마다 **새 셸**을 띄우므로 `source`로 export된 `_CFG_*`는 다음 Bash 호출에서 **빈 문자열**이 된다. 이를 방지하기 위해:

1. Preamble 0.1은 config를 `$HOME/.cache/team-agent/cfg-${_RUN_ID}.env`의 **persistent tempfile**에 기록한다 (`mktemp`가 아닌 결정적 경로 — Phase 5 종료 시 정리).
2. 이후 `_CFG_*`를 참조하는 **모든 Bash 블록은 선두에 다음 한 줄을 포함**한다:
   ```bash
   source "$HOME/.cache/team-agent/cfg-${_RUN_ID}.env" || { echo "[team-agent] FATAL: cfg.env 없음 — Preamble 0.1 미실행"; exit 1; }
   ```
3. Phase 5 완료 후 `rm -f "$HOME/.cache/team-agent/cfg-${_RUN_ID}.env"` + 오래된 cfg-*.env 정리.

> **왜 결정적 경로?** `mktemp`는 shell 로컬 변수만 반환하므로 cross-invocation에서 경로를 재구성할 수 없다. `$HOME/.cache/team-agent/` + `_RUN_ID` 조합은 예측 가능하면서 충돌 없고, chmod 600으로 권한 제한.

```bash
# Preamble 0.1: config 로드 → persistent cfg.env 저장 → 현재 shell에 source + sanity check.
_TA_CFG_DIR="$HOME/.cache/team-agent"
mkdir -p "$_TA_CFG_DIR" || { echo "[team-agent] FATAL: cache dir 생성 실패: $_TA_CFG_DIR" >&2; exit 1; }
chmod 700 "$_TA_CFG_DIR"
_TA_CFG_FILE="$_TA_CFG_DIR/cfg-${_RUN_ID}.env"
# symlink 선점 방어: 기존 파일이 symlink면 거부
if [ -L "$_TA_CFG_FILE" ]; then
  echo "[team-agent] FATAL: $_TA_CFG_FILE이 symlink — 중단" >&2
  exit 1
fi
# 기존 파일이 있으면 제거 후 빈 파일 chmod 600으로 생성
...
# cfg.env 마지막에 refs/gemini-helper.sh source 라인을 append. 이후 `source cfg.env`
# 한 줄만 호출하면 변수 + 함수 둘 다 바인딩된다.
# (fail-closed: helper.sh 없으면 후속 블록이 즉시 에러로 중단)
if [ ! -f "$_SKILL_DIR/refs/gemini-helper.sh" ]; then
  echo "[team-agent] FATAL: refs/gemini-helper.sh 누락 — cfg.env 완성 불가" >&2
  rm -f "$_TA_CFG_FILE"
  exit 1
fi
...
**_CFG_* 참조 블록 패턴** (Preamble 0.1 이후 모든 Bash 블록 적용):

```bash
# === Bash 블록 선두 (필수 — bug_007 방지) ===
source "$HOME/.cache/team-agent/cfg-${_RUN_ID}.env" 2>/dev/null || {
  echo "[team-agent] FATAL: cfg.env 없음 또는 source 실패 — Preamble 0.1이 먼저 실행되어야 함" >&2
  exit 1
}
...
> **round-5 C4 대응**: 함수 본체는 `${_SKILL_DIR}/refs/gemini-helper.sh`의 단일 진실원에 있다. Preamble 0.1의 cfg.env 생성 시 마지막에 `source "$_SKILL_DIR/refs/gemini-helper.sh"` 라인이 append되므로, 모든 후속 Bash 블록이 `source cfg.env` 한 줄만 호출하면 `_pick_gemini_model`과 `_run_with_timeout` 둘 다 바인딩된다. 이전엔 이 SKILL.md prose 블록에만 정의가 있어서 Bash 도구의 새 shell에선 `command not found`가 발생했다.

**시그니처 (refs/gemini-helper.sh 참조)**:

```text
_pick_gemini_model <role>
  role: "agent" | "verifier"
  stdout: 가용한 첫 모델명 (예: "gemini-3-flash-preview")
  stderr: skip된 후보의 rc/에러 요약
  rc 0: 성공 — stdout에 모델명
  rc 1: 전 후보 실패 — stdout **빈 문자열** (bug_006: caller는 반드시 rc 체크)
  
행위:
  - _CFG_GEMINI_AGENT_CANDIDATES 또는 _CFG_GEMINI_VERIFIER_CANDIDATES 배열 순회
  - 각 후보마다 `gemini -m X -p ping` probe
  - 429 RESOURCE_EXHAUSTED는 10초 대기 후 1회 재시도
  - zsh/bash 공통 `while IFS= read -r < <(tr ' ' '\n')` 패턴 (word-splitting 안전)
```
...
옵션:
  ...
  --ultra [full|selective]  3중 복제 전략 (기본: full)
                         ...
                         selective — 가중치별 선별 복제 + 2-replica baseline으로 ~25% 절감:
                                     ×1.5(정밀) 3중 / ×1.0(구조)·≤×0.7(문서·탐색) 모두 2중
...
**`ULTRA_STRATEGY=selective`**: 가중치 기반 선별 복제 + **2-replica baseline** (Codex round-3 #3)으로 비용 ~25% 절감. 이전 ~44% 수치는 ≤×0.7 역할을 1중으로 뒀을 때이며, 독립 검증 0회 문제로 폐기됨.

| 역할 가중치 | 복제 수 | 백엔드 |
|:---:|:---:|---|
| ×1.5 (정밀: 보안·디버거·성능·RAG·모델평가·퀀트 등) | 3중 | Claude + Codex + Gemini |
| ×1.0 (구조: 아키텍트·리뷰어·언어 전문가 등) | 2중 | Claude + Codex |
| ×0.7 (문서·모네타이제이션·OpenAPI 등) | 2중 | Claude + Codex |
| ×0.5 (탐색·QA·유저 리서치 등) | 2중 | Claude + Codex |

selective에서 CLI 다운그레이드: Codex 미설치 시 Gemini로 대체하여 2중 유지 (Phase 1 fail-safe 분기). Gemini 미설치 시 3중 역할은 Claude+Codex 2중으로 축소, 나머지는 그대로 Claude+Codex 2중. 양쪽 미설치 시 Phase 1에서 `ULTRA_MODE=false` 다운그레이드.

저장 스키마 (full·selective 공통):
- `agent_backends`: 평평한 딕셔너리. 존재하는 에이전트만 기록.
...
- selective: 역할별 실제 백엔드 집합 표시. 예: `Claude+Codex+Gemini (ultra:3)`, `Claude+Codex (ultra:2)`. 1중(`ultra:1`)은 가용성 극단(양쪽 CLI 없음) 예외 경로에만 발생하며, 이 경우 실제로는 `ULTRA_MODE=false`로 다운그레이드되어 별도 플로우로 진입.

Phase 2.5 통합자 처리 (2-replica baseline 이후):
- 정상 흐름: 모든 역할이 최소 2중이므로 통합자가 합의/모순 계산 수행.
- 예외 경로 (1중 기록): 다운그레이드 로직의 안전망 — 통합자 spawn 생략, 단일 결과를 `consensus_findings`로 직접 승격 (`agreement: "1/1"`). 정상 흐름에선 도달하지 않음.
...
manifest = {
    ...
    "ultra_strategy": ULTRA_STRATEGY_OR_NONE,
...
# bug_006: _pick_gemini_model은 전부 실패 시 빈 문자열 + rc=1 반환. 반드시 확인.
_GEMINI_AGENT_MODEL="$(_pick_gemini_model agent)"
if [ -z "$_GEMINI_AGENT_MODEL" ]; then
  echo "[team-agent] WARN: 가용한 Gemini agent 모델 없음 — 코드맵 생성 스킵 (에이전트 독립 탐색 폴백)" >&2
  _CODEMAP=""; _CODEMAP_RC=127
elif [ "$GEMINI_HAS_SCHEMA" -gt 0 ]; then
  _run_with_timeout "$_CFG_CODEMAP_SEC" 10 \
...
# bug_020 (round-5 C2): 위 if/elif/else 내부에서 이미 _CODEMAP_RC를 설정했다.
# 과거엔 여기에 `_CODEMAP_RC=$?` 고아 라인이 있었지만, compound command의 마지막
# assignment는 보통 rc=0을 반환하므로 timeout(124)/SIGKILL(137)/CLI 404 진단이
# 조용히 0으로 덮어써졌다. 이 라인은 의도적으로 삭제된다 — 각 분기 내부 할당만 유지.
```
...
# bug_006: 전부 실패 시 빈 문자열 + rc=1 반환. 반드시 확인하여 죽은 모델 재호출 방지.
_GEMINI_AGENT_MODEL="$(_pick_gemini_model agent)"
if [ -z "$_GEMINI_AGENT_MODEL" ]; then
  echo "[team-agent] gemini agent model 탐색 실패 (전 후보 404/429) — 이 에이전트 실패 처리" >&2
  _GEMINI_RC=127
elif [ "$GEMINI_HAS_SCHEMA" -gt 0 ]; then
  _run_with_timeout "$_CFG_AGENT_SOFT_SEC" "$_CFG_GRACE_SEC" \
    gemini -m "$_GEMINI_AGENT_MODEL" --json-schema "$_SCHEMA" \
