...
옵션:
  ...
  --ultra                각 역할에 Claude + Codex + Gemini 3중 배치 후 Opus가 역할별 통합
                         최고 정밀도 (3~4배 비용). --codex/--gemini/--cross 상호 배타
...
예시:
  ...
  /team-agent --ultra 프로덕션 감사      (각 역할에 Claude+Codex+Gemini, Opus 통합)
...
- `--ultra` → `ULTRA_MODE=true` (각 역할에 Claude+Codex+Gemini 3명 전부 배치, Opus가 역할별 통합). `--codex`·`--gemini`·`--cross`와 상호 배타. codex·gemini CLI 양쪽 필요 (미설치 시 자동 다운그레이드).
  - 라우팅: 가중치 무관, 모든 역할이 3중 replication
  - Phase 2.5에서 역할별 Opus 통합자가 3개 결과를 합성 (3/3·2/3·1/3 합의 계산, 모순 감지)
...
| `--ultra` + codex 미설치 | 경고 "codex CLI 미설치 — Ultra를 Claude+Gemini 2중으로 다운그레이드" |
| `--ultra` + gemini 미설치 | 경고 "gemini CLI 미설치 — Ultra를 Claude+Codex 2중으로 다운그레이드" |
| `--ultra` + 양쪽 미설치 | 경고 "외부 CLI 양쪽 미설치 — Ultra 무효, Claude 단독으로 진행" |
| `--ultra` + `--deep` | 허용. `--ultra`=역할 내 통합(Phase 2.5), `--deep`=역할 간 통합(Phase 3) — 계층 다름 |
| `--ultra` + `--resume` | 허용. manifest의 `agent_groups`·`ultra_mode` 복원 |
| `--ultra` + `--auto`/`--diff`/`--scope`/`--dry-run`/`--notify` | 모두 허용 |
...
with open("/tmp/ta-RUN_ID_VALUE-input.txt", encoding='utf-8') as f:
    raw = f.read()
# 유니코드 정규화 (NFKD) + hyphen 등가(U+2010~U+2015, U+00AD soft hyphen 등)를 ASCII '-'로 정규화
# 이렇게 하면 공격자가 '—BEGIN_USER_INPUT—' 같은 유니코드 hyphen으로 구분자를 우회할 수 없다
...
> **Override 사용법**: `TEAM_AGENT_META=true /team-agent …` 로 명시적 강제. `~/code/team-agent`, `~/projects/ta-fork` 같은 비표준 경로에서 메타 분석 원할 때.
...
에이전트 프롬프트에 해당 역할의 핵심 분석 초점을 반드시 포함한다. **상세 체크리스트는 `${_SKILL_DIR}/refs/checklists.md`를 Read하여 해당 역할 섹션만 추출**하고, 프롬프트의 `## 분석 체크리스트` 위치에 삽입한다. 아래는 역할-키워드 매핑 요약:
...
hybrid 라우팅 규칙은 무시된다. 각 역할마다 **Claude + Codex + Gemini 3명 전부**를 spawn한다 (가용 CLI에 따라 2중으로 다운그레이드 가능). 가중치(×0.5~×1.5)는 오직 비용 추정에만 사용된다.

저장 스키마:
- `agent_backends`: 여전히 평평한 딕셔너리. Ultra 모드에서는 `{역할}-claude`, `{역할}-codex`, `{역할}-gemini` 이름으로 각각 기록 (예: `security-auditor-claude: "claude"`).
- `agent_groups`: Ultra 전용 중첩 맵. `{역할이름: {"claude": "보안감사-claude", "codex": "보안감사-codex", "gemini": "보안감사-gemini"}}`. Phase 2.5 통합자가 역할별 3개 결과를 식별하는 데 사용.
- `agent_prompts`: 여전히 평평. `{에이전트이름: 프롬프트 전문}` 형태. Ultra에서도 동일.

Step 3-5 제안 테이블에서 Ultra 모드는 역할당 1행으로 표시하고 backend 컬럼을 `Claude+Codex+Gemini (ultra)` 또는 다운그레이드 시 `Claude+Codex (ultra↓)` / `Claude+Gemini (ultra↓)`로 표기.
...
## 분석 체크리스트
{Read 도구로 ${_SKILL_DIR}/refs/checklists.md에서 해당 역할 섹션을 추출하여 삽입. 통합 정합성 에이전트는 ${_SKILL_DIR}/refs/integration-qa.md 전문을 삽입}
...
각 역할(`team.roles`)마다 **3개 변형 프롬프트**를 동시 생성한다 (가용 CLI 수에 따라 2~3개):

1. **Claude 변형** (`{역할}-claude`): 기존 3-4 템플릿 그대로. Agent 도구 + `subagent_type: general-purpose`.
2. **Codex 변형** (`{역할}-codex`): `${_SKILL_DIR}/refs/codex-agent-template.md`로 탐색 지시를 셸 명령으로 치환. Bash + `codex exec -s read-only` (`ULTRA_CODEX_AVAIL=1` 일 때만).
3. **Gemini 변형** (`{역할}-gemini`): `${_SKILL_DIR}/refs/gemini-agent-template.md`로 치환. Bash + `gemini -p` (`ULTRA_GEMINI_AVAIL=1` 일 때만).

**Ultra 병렬 실행**:
- Claude 에이전트는 여전히 최대 3명씩 배치. 배치 간 5초 대기.
- Codex/Gemini 에이전트는 전원 `run_in_background`로 즉시 spawn (별도 프로세스).
- 최대 동시성: `(Claude 3) + (Codex ∞) + (Gemini ∞)` — 5역할 × 3프로바이더 = 15명이 거의 동시 실행 가능.
...
각 역할마다 **Opus 통합 에이전트**를 1명 생성해 해당 역할의 Claude/Codex/Gemini 3개 결과(다운그레이드 시 2개)를 합성한다. 검증 레이어(Phase 4-A-2)는 이 단계로 **대체**되므로 Ultra 모드에서 Phase 4-A-2는 스킵.
...
**실패 처리**:
- 특정 역할의 3중 결과가 모두 실패 → 해당 역할 `per_role_integration[{역할}] = {"error": "all agents failed"}` 기록 후 스킵.
- 통합자 자체 실패 → 재시도 1회. 재실패 시 3개 결과를 그대로 Phase 4-A-0으로 넘기되 `agreement: null`로 표시.
- 일부 에이전트만 성공 (예: 3중에서 Gemini만 성공) → 1/1 단독 입력으로 통합자 실행. `agreement: "1/1"`, `confidence: "medium"`로 기록.
...
- Codex + Gemini 동시 독립 실행 (`run_in_background`)
- `refs/cross-verification-schema.json`으로 구조화 출력
- Python 결정론적 매핑으로 2/3 합의 계산
- 결과를 `manifest.verification`에 기록

실패 폴백:
- Codex만 성공 → Codex 단독 판정 + "2/3 불가" 표시
- Gemini만 성공 → Gemini 단독 판정 + 동일 표시
- 둘 다 실패 → 검증 스킵, Claude 원본 채택 + "검증 실패" 경고
