...
2. **Codex 변형** (`{역할}-codex`): `${SKILL_DIR}/refs/codex-agent-template.md`로 탐색 지시를 셸 명령으로 치환. Bash + `codex exec -s read-only` (`ULTRA_CODEX_AVAIL=1` 일 때만).
3. **Gemini 변형** (`{역할}-gemini`): `${SKILL_DIR}/refs/gemini-agent-template.md`로 치환. Bash + `gemini -p` (`ULTRA_GEMINI_AVAIL=1` 일 때만).
