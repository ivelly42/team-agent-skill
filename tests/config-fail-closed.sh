...
# F1. mktemp 기반 tempfile
if echo "$PREAMBLE_0_1" | grep -qE 'mktemp.*ta-cfg-'; then
    pass "F1 — mktemp으로 secure tempfile 생성"
else
    fail "F1 — mktemp 기반 tempfile 누락 (predictable /tmp 경로 사용 중)"
fi
